...
import (
	...
	model2 "douyin-app/internal/pkg/model"
	"fmt"
...
type BearerAuthSrvInterface interface {
	SetJWTToken(ctx context.Context, user *model2.User) error
	CompareJWTToken(ctx context.Context, user *model2.User, token string) (bool, error)
	RenewJWTToken(ctx context.Context, user *model2.User) error
	DeleteJWTToken(ctx context.Context, user *model2.User) error
}
...
func (b bearerAuthService) SetJWTToken(ctx context.Context, user *model2.User) error {
	token, err := b.storeFactory.BearerAuthorization().GetJWTToken(ctx, user.Id)
	// 如果已经存在 token，直接返回
	if err == nil {
		return fmt.Errorf("user_id 已经存在 Redis 中，无法创建")
	}
	myClaim, err := model2.NewJWTClaims(user.Id)
...
func (b bearerAuthService) CompareJWTToken(ctx context.Context, user *model2.User, token string) (bool, error) {
...
func (b bearerAuthService) RenewJWTToken(ctx context.Context, user *model2.User) error {
...
func (b bearerAuthService) DeleteJWTToken(ctx context.Context, user *model2.User) error {
